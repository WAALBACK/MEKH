  ...
  useEffect(() => {
    const fetchNearbyTechnicians = async () => {
      ...
      try {
        ...
        // Strategy 1: Area name matching (fuzzy/partial match) - fetch all for area matching
        const { data: allTechnicians, error: fetchError } = await supabase
          ...
          .select(`
            ...
            trusted_brands_used, service_guarantee,
            status, user_id, profile_image, cover_photo, thumbnail_image, tow_truck_number_plate, created_at,
            latitude, longitude, google_maps_link,
            avg_rating, review_count, completed_jobs, is_verified,
            technician_services(
              id, technician_id, service_name, category, price, negotiable, notes, is_primary,
        ...
        const technicians = (allTechnicians || []) as unknown as Technician[];

        const areaMatches = technicians.filter(tech =>
          areasMatch(current.area, tech.area)
        );

        let proximityMatches: (Technician & { distance?: number })[] = [];
        if (current.latitude && current.longitude) {
          const { data: proximityData, error: proximityError } = await supabase.rpc('get_nearby_technicians', {
            user_lat: current.latitude,
            user_lon: current.longitude,
          ...
          // The RPC only returns id + distance_km — merge with full technician data
          // that was already fetched in allTechnicians above
          const distanceMap = new Map<string, number>();
          (proximityData || []).forEach((item: { id: string; distance_km: number }) => {
            distanceMap.set(item.id, item.distance_km);
          });

          proximityMatches = technicians
            .filter(tech => distanceMap.has(tech.id))
            .map(tech => ({
              ...tech,
              distance: distanceMap.get(tech.id),
            }));
        }
        ...
        // Add proximity matches first with distance data
        proximityMatches.forEach(tech => {
          combinedMatches.set(tech.id, tech);
        });

        // Add area matches — attach distance if already in map, skip if duplicate
