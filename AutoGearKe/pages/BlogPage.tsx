      ...
      <Helmet>
        ...
        <script type="application/ld+json">
          {JSON.stringify({
            ...
            '@type': 'Blog',
            name: 'Mekh Insights',
            description: 'Expert tips on car tinting, wrapping, PPF, ceramic coating, and detailing in Kenya. Your go-to source for car care advice from Mekh.',
            ...
            mainEntity: articles.slice(0, 10).map(article => ({
              ...
              url: `https://mekh.app/blogs/${article.slug}`,
              ...
              ...(article.images?.[0]?.url && {
                image: cx(article.images[0].url, 'w_1200,h_630,c_fill,q_auto,f_auto')
              })
            }))
          })}
        </script>
                    ...
                    <div className="hidden md:block absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/40 to-transparent" />
                      {/* Text overlay — md and up only */}
                       <div className="hidden md:block absolute bottom-0 left-0 right-0 p-8">
                        <h2 className="text-2xl lg:text-3xl font-black text-blue-500 leading-tight mb-2">
                           {featuredArticle.title}
                         </h2>
                         {featuredArticle.excerpt && (
                        <p className="text-white text-base line-clamp-2 max-w-2xl">{featuredArticle.excerpt}</p>
                         )}
