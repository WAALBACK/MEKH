...
const AuthPage: React.FC<AuthPageProps> = ({ onAuthSuccess }) => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // On native the branded loader stays visible while the external browser is open.
      // The deep-link listener in App.tsx will navigate away when the flow completes.
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 py-8 px-4">
      ...
      <div className="max-w-md mx-auto">
        ...
        {/* Role Selection (after Google sign up) */}
        {showRoleSelection && (
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
            <div className="text-center">
              <h3 className="text-lg font-bold text-white mb-2">Welcome to Mekh!</h3>
              <p className="text-slate-400 text-sm mb-6">How would you like to use Mekh?</p>
              
              <div className="space-y-3">
                <button
                  onClick={() => handleRoleSelect('client')}
                  className="w-full p-4 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-xl text-left transition-colors"
                >
                  <div className="font-bold text-white">I'm a Client</div>
                  <div className="text-slate-400 text-sm">Looking for automotive services</div>
                </button>
