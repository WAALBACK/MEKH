...
import { VerificationProgress } from '../src/components/VerificationProgress';
import { profileFull, portfolioThumb } from '../src/lib/cloudinary';
import { takeOrPickPhoto } from '../src/lib/camera';
...
const TechnicianDashboardPage: React.FC = () => {
  ...
  const [servicesForm, setServicesForm] = useState<{ id?: string; service_name: string; category: ServiceCategory | ''; price: string; negotiable: boolean; notes?: string; is_primary: boolean; variants: { id?: string; service_id?: string; variant_name: string; price: string; is_negotiable: boolean }[] }[]>([]);
  ...
  // 24/7 mode state
  const [isBusinessHours24_7, setIsBusinessHours24_7] = useState<boolean>(false);
  ...
  // Sync activeTab with URL search params and reload data when tab changes
  useEffect(() => {
    const tabParam = searchParams.get('tab') as TabType;
    if (tabParam && ['profile', 'services', 'bookings', 'notifications', 'settings', 'technicians'].includes(tabParam)) {
      setActiveTab(tabParam);
      // Reload data when tab changes to load tab-specific data
      if (technician) {
  ...
  const loadTabData = async (tab: TabType) => {
    ...
    try {
      if (tab === 'services' && serviceVariants.length === 0) {
        const [variantsData, allServices] = await Promise.all([
          supabase
            .from('service_variants')
            .select('*')
            .in('service_id', technician.technician_services?.map((s: TechnicianService) => s.id) || [])
            .then(res => res.data),
          getAllServices().catch(() => ALL_SERVICES.map(name => ({ name })))
        ]);

        setServiceVariants(variantsData || []);
        
        // Deduplicate services with exact character-by-character matching
        const serviceNames = allServices.map(s => typeof s === 'string' ? s : s.name);
        const uniqueServices = Array.from(new Set(serviceNames));
        setAvailableServices(uniqueServices);
      }
      
      if ((tab === 'profile' || tab === 'services') && videos.length === 0) {
        const [videosData, hoursData] = await Promise.all([
          supabase
        ...
        if (hoursData && hoursData.length > 0) {
          const mappedHours = DAYS_OF_WEEK.map(day => {
            ...
            return {
              ...
              available_on_request: existing.available_on_request ?? false,
            };
          });
      ...
      if (tab === 'bookings' && leads.length === 0) {
      ...
      if (tab === 'technicians' && allTechnicians.length === 0) {
  ...
  const loadData = async () => {
    try {
      ...
      setProfileForm({
        ...
        trusted_brands_used: techProfile.trusted_brands_used || '',
        service_guarantee: techProfile.service_guarantee || '',
        profile_image: techProfile.profile_image || '',
        cover_photo: techProfile.cover_photo || '',
        thumbnail_image: techProfile.thumbnail_image || '',
        google_maps_link: techProfile.google_maps_link || '',
        tow_truck_number_plate: techProfile.tow_truck_number_plate || '',
      });
      ...
      // Load tab-specific data based on active tab to improve initial load
      if (activeTab === 'services') {
        // Load service variants and available services only when on services tab
        const [variantsData, allServices] = await Promise.all([
          supabase
            .from('service_variants')
            .select('*')
        ...
        setServiceVariants(variantsData || []);
        setAvailableServices(allServices.map(s => typeof s === 'string' ? s : s.name));
      }
      
      if (activeTab === 'profile' || activeTab === 'services') {
        // Load videos and business hours when on profile or services tab (videos shown in services)
        const [videosData, hoursData] = await Promise.all([
          supabase
            .from('technician_videos')
            .select('*')
...
if (hoursData && hoursData.length > 0) {
           ...
           // Check if all hours are set to 24/7 format
           const isAll24Hours = mappedHours.every(h => 
             h.is_open && 
             h.open_time === '00:00' && 
             h.close_time === '23:59'
           );
           setIsBusinessHours24_7(techProfile.is_24_7 || isAll24Hours);
         } else {
           ...
           setIsBusinessHours24_7(false);
         }
      } else if (activeTab === 'bookings') {
        // Load leads only when on bookings tab
        const myLeads = await getMyLeads();
        setLeads(myLeads);
      } else if (activeTab === 'notifications') {
        // Load notifications only when on notifications tab
        const myNotifications = await getMyNotifications();
        setNotifications(myNotifications);
      } else if (activeTab === 'technicians') {
        // Load all technicians only when on technicians tab (admin view)
        const allTechs = await getTechnicians();
        setAllTechnicians(allTechs);
      }
      
    } catch (err) {
      console.error('🔧 TECHNICIAN DASHBOARD DEBUG: Error loading data:', err);
  ...
  const handleProfileSave = async () => {
    try {
      ...
      // Validate tow truck number plate if technician offers towing services
      const offersTowing = services.some(s => s.category === 'towing');
      if (offersTowing && !profileForm.tow_truck_number_plate.trim()) {
        setError('Tow truck number plate is required for technicians offering towing services');
        setSaving(false);
        return;
      }
      ...
      await updateMyProfile({
        ...
        trusted_brands_used: profileForm.trusted_brands_used,
        service_guarantee: profileForm.service_guarantee,
        ...
        tow_truck_number_plate: profileForm.tow_truck_number_plate,
        is_24_7: isBusinessHours24_7,
      });
...
// Save business hours
       if (technician) {
         ...
         if (isBusinessHours24_7) {
           // When in 24/7 mode, set all days to open 24 hours
           hoursToSave = DAYS_OF_WEEK.map(day => ({
           ...
           hoursToSave = businessHours.map(h => ({
             technician_id: technician.id,
  ...
  const handleProfileImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const url = await imageUpload.uploadImage(file, 'technicians/profiles');
    ...
    if (url) {
      ...
      setError(imageUpload.error || 'Upload failed. Please check your connection and try again.');
  ...
  const handleCoverPhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const url = await imageUpload.uploadImage(file, 'technicians/covers');
  ...
  const handleThumbnailUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const url = await imageUpload.uploadImage(file, 'technicians/thumbnails');
  ...
  const handleServicesSave = async () => {
    try {
      ...
      const servicesToUpsert: { id?: string; technician_id: string; service_name: string; category: ServiceCategory | ''; price?: number | null; negotiable: boolean; notes?: string | null; is_primary: boolean }[] = [];
      const primaryService = servicesForm.find(s => s.is_primary)?.service_name || '';
      servicesForm.forEach((s) => {
        if (s.variants.length > 0) {
          ...
          servicesToUpsert.push({
            ...
            negotiable: false,
            notes: s.notes || null,
            is_primary: s.service_name === primaryService,
          });
        } else {
  ...
  // Helper function to get available services for dropdown, excluding already selected ones
  const getAvailableServicesForDropdown = (currentIndex: number): string[] => {
    // Get services already selected in the form (excluding current service being edited)
    const selectedServices = servicesForm
      .map((s, idx) => idx !== currentIndex && s.service_name ? s.service_name : null)
      .filter((s): s is string => s !== null);

    // Filter out already selected services with exact character-by-character matching
    return availableServices.filter(service => !selectedServices.includes(service));
  };
  ...
  const startEditingServices = () => {
    ...
    const servicesWithVariants: { id?: string; service_name: string; category: ServiceCategory; price: string; negotiable: boolean; notes?: string; is_primary: boolean; variants: { id?: string; service_id?: string; variant_name: string; price: string; is_negotiable: boolean }[] }[] = [];
  ...
  const handleGetCurrentLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        setProfileForm(prev => ({ ...prev, google_maps_link: googleMapsLink }));
        ...
        try {
          ...
          setSuccess(`Location resolved: ${locationName}`);
        } catch (err) {
          setResolvedLocationName(`${latitude}, ${longitude}`);
          setSuccess(`Location coordinates: ${latitude}, ${longitude}`);
        } finally {
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const urls = await imageUpload.uploadImages(filesArray, 'technicians/portfolio');
      
      if (urls.length > 0) {
        // Create new photo objects
        const newPhotos: TechnicianPhoto[] = urls.map((url, index) => ({
          id: '',
          technician_id: technician?.id || '',
          photo_url: url,
          caption: '',
        ...
        // Save to Supabase immediately after upload completes
        await updateMyPhotos(updatedPhotos.map(p => ({
          id: p.id || undefined,
        ...
        setPhotosRemoved(false);
        setSuccess(`${urls.length} photo(s) uploaded and saved successfully!`);
        setTimeout(() => setSuccess(''), 3000);
      ...
      setError(err.message || 'Failed to upload and save photos');
      setTimeout(() => setError(''), 5000);
    } finally {
      setSaving(false);
    }
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-lg p-6">
              ...
              <div className="mb-6">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col gap-2">
                    <label className="cursor-pointer px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-600/50 text-white text-sm rounded-lg transition-colors">
                      {imageUpload.isUploading ? 'Uploading...' : 'Change Photo'}
                      <input 
                        type="file" 
                        accept="image/*" 
                    ...
                    {imageUpload.isUploading && (
                      <span className="text-xs text-slate-400">{imageUpload.progress}%</span>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col gap-2">
                    <label className="cursor-pointer px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-600/50 text-white text-sm rounded-lg transition-colors">
                      {imageUpload.isUploading ? 'Uploading...' : (profileForm.thumbnail_image ? 'Change Thumbnail' : 'Upload Thumbnail')}
                      <input 
                        type="file" 
                        accept="image/*" 
              ...
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  {resolvedLocationName && !locationError && (
               ...
               {/* Tow Truck Details - Only show if technician offers towing services */}
               {services.some(s => s.category === 'towing') && (
                 <div className="mt-6">
                   <label htmlFor="towTruckNumberPlate" className="block text-sm font-medium text-slate-400 mb-1">
                     Tow Truck Number Plate <span className="text-red-400">*</span>
                   </label>
                   <input
                     id="towTruckNumberPlate"
                     type="text"
                     value={profileForm.tow_truck_number_plate}
                     onChange={(e) => setProfileForm({ ...profileForm, tow_truck_number_plate: e.target.value.toUpperCase() })}
                     placeholder="e.g. KCA 123A"
                     className="w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded-lg text-slate-500 focus:outline-none focus:border-blue-500 uppercase"
                     required
                   />
                   <p className="text-xs text-slate-500 mt-1">
                     Required for towing services - used for verification and customer trust
                   </p>
                   {!profileForm.tow_truck_number_plate && (
                     <p className="text-red-500 text-xs mt-1">Tow truck number plate is required for towing services</p>
                   )}
                 </div>
               )}
             </div>
...
<BusinessHoursEditor
                 ...
                 is24_7={isBusinessHours24_7}
                 on24_7Change={setIsBusinessHours24_7}
               />
            </div>

            {/* Pricing Notes */}
            <div className="bg-slate-900 border border-slate-800 rounded-lg p-6">
              <h2 className="text-lg text-blue-500 font-bold mb-4">Announcements</h2>
              <label htmlFor="pricingNotes" className="sr-only">Announcements</label>
              <textarea
                id="pricingNotes"
                ...
                rows={3}
                className="w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded-lg text-slate-500 focus:outline-none focus:border-blue-500"
                placeholder="e.g If you have offers and price increaments.."
              />
            </div>

            {/* Trusted Brands Used & Service Guarantee — hidden for towing technicians */}
            {!services.some(s => s.is_primary && s.category === 'towing') && (
              <>
                {/* Trusted Brands Used */}
                <div className="bg-slate-900 border border-slate-800 rounded-lg p-6">
                  <h2 className="text-lg text-blue-500 font-bold mb-4">Trusted Brands Used</h2>
                  <label htmlFor="trustedBrandsUsed" className="sr-only">Trusted brands used</label>
                  <input
                    id="trustedBrandsUsed"
                    type="text"
                    value={profileForm.trusted_brands_used}
                    onChange={(e) => setProfileForm({ ...profileForm, trusted_brands_used: e.target.value })}
                    className="w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded-lg text-slate-500 focus:outline-none focus:border-blue-500"
                    placeholder="3M, Pioneer, JBL, LLumar"
                  />
                </div>

                {/* Service Guarantee */}
                <div className="bg-slate-900 border border-slate-800 rounded-lg p-6">
                  <h2 className="text-lg text-blue-500 font-bold mb-4">Service Guarantee</h2>
                  <label htmlFor="serviceGuarantee" className="sr-only">Service guarantee</label>
                  <textarea
                    id="serviceGuarantee"
                    value={profileForm.service_guarantee}
                    onChange={(e) => setProfileForm({ ...profileForm, service_guarantee: e.target.value })}
                    rows={4}
                    className="w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded-lg text-slate-500 focus:outline-none focus:border-blue-500"
                    placeholder="7-day correction guarantee&#10;Warranty available&#10;Free re-installation if issue occurs"
                  />
                </div>
              </>
            )}
              ...
              {editingServices ? (
                <div className="space-y-4">
                  {servicesForm.map((service, index) => (
                    <div key={index} className="p-4 bg-slate-800 rounded-lg">
                      <div className="flex items-start gap-4">
                          ...
                          <div className="md:col-span-2">
                            ...
                            <div className="space-y-2">
                              ...
                              >
                                ...
                                {getAvailableServicesForDropdown(index).map(s => (
                        ...
                        <div className="mt-3">
                          <label htmlFor={`service-category-${index}`} className="block text-xs text-blue-500 mb-1">
                  ...
                  {servicesForm.length > 0 && (
                    <div className="p-4 bg-slate-800 rounded-lg border-2 border-blue-600">
                      <label htmlFor="primary-service-select" className="block text-sm text-blue-500 mb-2">
                        Primary Service <span className="text-red-400">*</span>
                      </label>
                      <p className="text-slate-400 text-xs mb-3">
                        Select your main service. This will be displayed prominently on your profile and in search results.
                      </p>
                      <select
                        id="primary-service-select"
                        aria-label="Primary Service"
                        value={servicesForm.find(s => s.is_primary)?.service_name || ''}
                        onChange={(e) => {
                          const updated = servicesForm.map(s => ({
                            ...s,
                            is_primary: s.service_name === e.target.value
                          }));
                          setServicesForm(updated);
                          // Mark all services as dirty since we're changing is_primary
                          setDirtyServiceIndices(new Set(servicesForm.map((_, idx) => idx)));
                        }}
                      ...
                      {!servicesForm.some(s => s.is_primary) && servicesForm.length > 0 && (
                        <p className="text-yellow-500 text-xs mt-2">⚠️ Please select a primary service</p>
                      )}
                    </div>
                  )}
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-lg p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-2">
                  <label className={`cursor-pointer px-4 py-2 ${saving || imageUpload.isUploading ? 'bg-blue-600/50 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'} text-white text-sm rounded-lg transition-colors`}>
                    {saving || imageUpload.isUploading ? (photosRemoved ? 'Deleting...' : 'Uploading...') : 'Add Photos'}
                    <input type="file" accept="image/*" multiple onChange={handlePhotoUpload} className="hidden" disabled={saving || imageUpload.isUploading} />
                  </label>

                   {/* Camera button for native app only - COMMENTED OUT FOR FUTURE USE */}
                   {/* {isNative && (
                     <button
                       type="button"
                       onClick={async () => {
                       ...
                       disabled={saving || imageUpload.isUploading}
                       className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-600/50 text-white text-sm rounded-lg transition-colors flex items-center gap-1"
                     >
                       Take Photo
                     </button>
                   )} */}
              ...
              {(saving || imageUpload.isUploading) && !photosRemoved && (
                <div className="mb-4 p-3 bg-blue-900/30 border border-blue-800 text-blue-400 rounded-lg">
                  <p className="text-sm">Uploading and saving photos... Please wait.</p>
                  {imageUpload.progresses.length > 0 && (
                    <div className="mt-2 space-y-1">
                      {imageUpload.progresses.map((prog, idx) => (
              ...
              {photos.length > 0 && (
                ...
                >
                  {saving ? (photosRemoved ? 'Deleting...' : 'Saving...') : 'Save Photo Changes'}
                </button>
              )}
              
              <p className="text-xs text-slate-500 mt-3">
                Photos are automatically saved after upload. Use "Save Photo Changes" only if you remove photos or edit captions.
              </p>
            </div>
