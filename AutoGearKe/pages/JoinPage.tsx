...
import { uploadToCloudinary, getTikTokThumbnail } from '../src/lib/api';
...
const JoinPage: React.FC = () => {
  ...
  // 24/7 mode state for business hours
    ...
    const checkAuth = async () => {
      try {
        ...
        if (savedState) {
          ...
          setPrimaryService(savedState.primaryService || '');
  ...
  const handleServiceToggle = (service: string) => {
    if (selectedServices.includes(service)) {
      ...
      // Clear primary service if it was the removed service
      ...
      // Auto-select as primary if it's the first service
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>, index: number) => {
    ...
    // Find available slots starting from the clicked index
    const availableSlots: number[] = [];
    let currentIndex = index;
    
    for (let i = 0; i < filesArray.length && availableSlots.length < 6; i++) {
      while (currentIndex < 6 && newSlots[currentIndex] !== null) {
        currentIndex++;
      }
      if (currentIndex < 6) {
        availableSlots.push(currentIndex);
        currentIndex++;
      }
    }
    
    // Check if we can upload all files
    if (availableSlots.length < filesArray.length) {
      setError(`Only ${availableSlots.length} of ${filesArray.length} images can be uploaded. All 6 slots will be filled.`);
      setTimeout(() => setError(''), 5000);
    }
    
    // Upload only the files that fit
    const filesToUpload = filesArray.slice(0, availableSlots.length);
    imageUpload.reset();
    
    const urls = await imageUpload.uploadImages(filesToUpload, 'technicians/portfolio');
    
    // Fill slots with successful uploads
    urls.forEach((url, i) => {
      const slotIndex = availableSlots[i];
      newSlots[slotIndex] = { url, file: filesToUpload[i] };
    });
  ...
  const saveProfileStep = async (): Promise<boolean> => {
    ...
    try {
      ...
      const insertData = {
        ...
        slug,
        status: 'live',
        tow_truck_number_plate: profileForm.tow_truck_number_plate || null,
      };
    ...
    try {
      ...
      for (const serviceName of selectedServices) {
        ...
        servicesToUpsert.push({
          ...
          is_primary: serviceName === primaryService,
        });
      }
  ...
  const savePhotosStep = async (): Promise<boolean> => {
    ...
    // Check if there are any upload errors from the image upload hook
    if (imageUpload.error) {
      setError('Some images failed to upload. Please try uploading them again.');
      setTimeout(() => setError(''), 5000);
      return false;
    }

    // Verify all photo slots have valid URLs (successfully uploaded)
    const hasInvalidPhotos = photoSlots.some(slot => slot !== null && (!slot.url || slot.url.trim() === ''));
    if (hasInvalidPhotos) {
      setError('Some images were not uploaded successfully. Please remove and re-upload them.');
      setTimeout(() => setError(''), 5000);
      return false;
    }
    ...
    try {
      ...
      if (photosToInsert.length > 0) {
        const { error: insertError } = await supabase.from('technician_photos').insert(photosToInsert);
        
        if (insertError) {
          console.error('Error inserting photos:', insertError);
          throw new Error('Failed to save photos to database. Please try again.');
        }
      }
  ...
  const saveVideosStep = async (): Promise<boolean> => {
    ...
    try {
      ...
      // Fetch thumbnails for TikTok videos before inserting
      const videosToInsert = await Promise.all(videoLinks.map(async (v) => {
        let thumbnailUrl: string | null = null;
        if (v.platform === 'tiktok') {
          try {
            thumbnailUrl = await getTikTokThumbnail(v.url);
          } catch (err) {
            console.error('[WIZARD] Failed to fetch TikTok thumbnail:', err);
            // Continue even if thumbnail fetch fails
          }
        }
        return {
          technician_id: technicianId,
          ...
          thumbnail_url: thumbnailUrl, // Cache thumbnail in DB
        };
      }));
  ...
  const savePricingStep = async (): Promise<boolean> => {
    ...
    try {
      ...
      if (isBusinessHours24_7) {
        hoursToInsert = DAYS_OF_WEEK.map(day => ({
          technician_id: technicianId,
          day_of_week: day.value,
          is_open: true,
          open_time: '00:00',
        ...
        hoursToInsert = businessHours.map(h => ({
          technician_id: technicianId,
  ...
  const handleContinue = async () => {
    ...
    if (success) {
      ...
      if (nextIndex < STEPS.length) {
        ...
        // Trigger auth cache invalidation to refresh user role and show technician bottom navbar
        window.dispatchEvent(new CustomEvent('invalidate-auth-cache'));
        
        // Also trigger user signed in event to ensure UI updates
        window.dispatchEvent(new CustomEvent('user-signed-in', { 
          detail: { session: { user } } 
        }));
        
        // Navigate to technician dashboard after a short delay
        setTimeout(() => {
          navigate('/technician-dashboard');
        }, 2000);
      }
  ...
  const isStepValid = (): boolean => {
    switch (currentStep) {
      ...
      case 'services':
        // Check if towing category is selected and tow truck details are provided
        const hasTowingCategory = Object.values(selectedCategories).includes('towing');
        if (hasTowingCategory && !profileForm.tow_truck_number_plate.trim()) {
          return false;
        }
          ...
          <div className="space-y-4">
                ...
                {selectedServices.map(serviceName => (
                  <div key={serviceName}>
                    ...
                    {serviceName === 'Window Tinting' ? (
                      <>
                        {/* Service Category Selection - Required for Window Tinting */}
                        <div className="mb-4 p-4 bg-slate-900 rounded-lg border border-blue-600">
                          ...
                          <select
                            ...
                            className="w-full px-3 py-2 bg-slate-900 border border-blue-600 rounded-lg text-slate-500 focus:outline-none focus:border-blue-500 mb-3"
                            required
                          >
                            ...
                            <option value="towing">Towing (towing, tow, recovery, flatbed, jumpstart)</option>
                          </select>
                          {!selectedCategories[serviceName] && (
                            <p className="text-red-500 text-xs mb-3">Please select a category for this service</p>
                          )}
                       ...
                       <div className="mb-3">
                         <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                           <div className="flex items-center gap-2 sm:w-32">
                             <span className="text-blur-500 text-xs">{serviceName}</span>
                             <button
                               type="button"
                              ...
                              <>
                                <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                                  <input
                                    ...
                                    className="flex-1 px-3 py-2 bg-slate-900 border border-blue-600 rounded-lg text-blue-500 text-sm"
                                  />
            ...
            {selectedServices.length > 0 && (
              <div className="mt-6 pt-4 border-t border-slate-700">
                ...
                {!primaryService && selectedServices.length > 0 && (
                  <p className="text-yellow-500 text-xs mt-2">⚠️ Please select a primary service to continue</p>
                )}
              </div>
            )}

            {/* Tow Truck Details - Required if towing category is selected */}
            {Object.values(selectedCategories).includes('towing') && (
              <div className="mt-6 pt-4 border-t border-slate-700">
                <label className="block text-sm text-blue-500 mb-1">
                  Tow Truck Number Plate <span className="text-yellow-400">*</span>
                </label>
                <p className="text-slate-400 text-xs mb-3">
                  Since you offer towing services, please provide your tow truck's number plate. This is required for verification and customer trust.
                </p>
                <input
                  type="text"
                  ...
                  className="w-full px-4 py-3 bg-slate-900 border border-blue-600 rounded-lg text-slate-500 focus:outline-none focus:border-blue-500 uppercase"
                />
      ...
      case 'photos':
        return (
          <div className="space-y-4">
                ...
                <div key={index} className="relative aspect-square">
                  {slot ? (
                    <div className="w-full h-full rounded-lg overflow-hidden relative group">
                     ...
                     <div className="w-full h-full border-2 border-dashed border-blue-600 hover:border-blue-500 rounded-lg flex flex-col items-center justify-center cursor-pointer transition-colors bg-slate-900">
            ...
            {imageUpload.isUploading && imageUpload.progresses.length > 0 && (
              <div className="space-y-2">
                <p className="text-sm text-blue-500">
                  Uploading {imageUpload.progresses.filter(p => p === 100).length} of {imageUpload.progresses.length} photos...
                </p>
                <div className="grid grid-cols-3 gap-2">
                  {imageUpload.progresses.map((progress, i) => (
      ...
      case 'pricing':
        return (
          <div className="space-y-4">
            ...
            <p className="text-slate-500 text-sm mb-6">Add any information including offers (optional)</p>
  ...
  return (
    <div className="min-h-screen bg-slate-950 py-8 px-4">
      ...
      <div className="max-w-3xl mx-auto">
        ...
        {/* Internal Links Section - Only show in web version, not in native app */}
        {currentStep !== 'complete' && !Capacitor.isNativePlatform() && (
