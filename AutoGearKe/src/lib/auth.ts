...
export const signInWithGoogle = async () => {
  ...
  const redirectTo = isNativePlatform
    ? 'com.mekh.app://auth/callback'
    : window.location.origin;

  // Set OAuth in progress flag for native apps
  if (isNativePlatform) {
    window.dispatchEvent(new CustomEvent('oauth-started'));
  }
  ...
  const { data, error } = await supabase.auth.signInWithOAuth({
    ...
    options: { 
      redirectTo,
      skipBrowserRedirect: isNativePlatform, // Only skip on native
    },
  });
  if (error) {
    if (isNativePlatform) {
  ...
  // Force open in system browser on native platforms only
...
export const signUpTechnicianWithGoogle = async () => {
  ...
  const isNativePlatform = Capacitor.isNativePlatform();
  const redirectTo = isNativePlatform
    ? 'com.mekh.app://auth/callback?role=technician'
...
export const signUpTechnician = async (email: string, password: string) => {
  const { data, error } = await supabase.auth.signUp({
    ...
    password,
    options: {
      emailRedirectTo: 'https://mekh.app/auth/callback',
...
export const signOut = async () => {
  try {
    ...
    // Clear auth cache and dispatch events for React Query invalidation
    const { clearCachedAuthState } = await import('./authCache');
    clearCachedAuthState();
    
    // Dispatch sign-out event for React Query to invalidate caches
    ...
    try {
      ...
      console.error('[SIGNOUT] Failed to clear cache:', cacheError);
    }
