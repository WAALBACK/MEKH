...
/**
 * Lightweight technician fetch for homepage cards.
 * Only selects columns needed for TechnicianCard rendering.
 * ~40-60% smaller JSON payload vs getPublicTechnicians.
 * Uses in-memory cache with stale-while-revalidate.
 */
export const getPublicTechniciansLite = async (): Promise<any[]> => {
  return apiCache.fetchWithCache(
    CACHE_KEYS.PUBLIC_TECHNICIANS_LITE,
    async () => {
      const { quality } = getConnectionQuality();
      // Connection-aware limit: fetch fewer records on slow connections
      const limit = quality === 'slow' ? 12 : quality === 'moderate' ? 20 : 50;

      const { data, error } = await supabase
        .from('technicians')
        .select(`
          id, business_name, slug, phone, area, county, mobile_service, is_24_7,
          status, profile_image, cover_photo, thumbnail_image, created_at,
          latitude, longitude,
          avg_rating, review_count, completed_jobs, is_verified,
          technician_services(
            id, service_name, category, price, negotiable, is_primary,
            service_variants(id, variant_name, price, is_negotiable)
          )
        `)
        .eq('status', 'live')
...
/**
 * Lightweight fetch for homepage cards using PostGIS to find nearby technicians.
 */
export const findNearbyTechnicians = async (lat: number, lng: number): Promise<any[]> => {
  const { quality } = getConnectionQuality();
  const limit = quality === 'slow' ? 12 : quality === 'moderate' ? 20 : 50;
  ...
  // Map the output to match the expected format (mapping distance_km to distance)
  return (data || []).map(tech => ({
    ...tech,
    distance: tech.distance_km // Map it so frontend components using t.distance keep working
  }));
};


/**
 * Cached wrapper for getPublicArticles.
 * Articles change infrequently — safe to cache for a few minutes.
 */
export const getPublicArticlesCached = async (): Promise<any[]> => {
  return apiCache.fetchWithCache(
    CACHE_KEYS.PUBLIC_ARTICLES,
    async () => {
      const { data, error } = await supabase
        .from('articles')
        .select('id, slug, title, excerpt, is_published, created_at')
...
export const getPublicTechnicians = async (filters?: { 
  area?: string; 
  service?: string;
  limit?: number;
  lastId?: string;
  ...
  // Use keyset pagination for better performance on slow connections
  ...
  let query = supabase
    ...
    .select(`
      ...
      county, area, mobile_service, instagram, tiktok_link, youtube_link, pricing_notes, is_24_7,
  ...
  // Keyset pagination: fetch records after the last seen record
  if (filters?.lastId && filters?.lastCreatedAt) {
    query = query.or(`created_at.lt.${filters.lastCreatedAt},and(created_at.eq.${filters.lastCreatedAt},id.lt.${filters.lastId})`);
  }
...
export const getPublicTechnicianBySlug = async (slug: string): Promise<any> => {
  const { data, error } = await supabase
    ...
    .select(`
      ...
      trusted_brands_used, service_guarantee, is_24_7,
      status, user_id, profile_image, cover_photo, thumbnail_image, created_at,
      latitude, longitude, google_maps_link,
      avg_rating, review_count, completed_jobs, is_verified,
      technician_services(
...
export const getMyClientLeads = async (_userId?: string) => {
  ...
  const { data, error } = await supabase
    .from('leads')
    .select('*, technicians(business_name, slug, profile_image, first_name, last_name, phone)')
...
/**
 * Upload a file to Cloudinary via its unsigned upload API with compression, timeout, and retry logic.
 * Returns the secure URL of the uploaded image.
 * 
 * @param file - The image file to upload
 * @param folder - The Cloudinary folder path (default: 'technicians/profiles')
...
export const uploadToCloudinary = async (
  ...
  // 2b. Compress the image before upload
  ...
  try {
    ...
    compressedFile = await imageCompression(file, {
      ...
      // Disable web worker in development to avoid CSP issues with Vite dev server
      // In production, the CSP headers in _headers file allow blob: for workers
      useWebWorker: import.meta.env.PROD,
      fileType: 'image/webp',
      initialQuality: 0.8,
    });
  ...
  // 2c & 2d. Upload with timeout + retry using XMLHttpRequest for progress tracking
  const uploadWithRetry = async (attemptNumber: number = 1): Promise<string> => {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      const url = await uploadWithRetry(attempt);
      return url;
    } catch (error: any) {
      lastError = error;
      
      // Don't retry on 4xx errors (bad request, auth failure, etc.)
      if (error.message.includes('Cloudinary upload failed: 4')) {
        throw error;
      }
...
/**
 * Find nearby emergency technicians based on client location and mobility status.
 * All filtering, sorting, distance calculation, and ETA computation is done server-side.
 */
export const findNearbyEmergencyTechnicians = async (params: {
  lat: number;
  lng: number;
  mobilityStatus: 'yes' | 'barely' | 'no';
}): Promise<any[]> => {
  const { data, error } = await supabase.rpc('find_nearby_emergency_technicians', {
    p_lat: params.lat,
    p_lng: params.lng,
    p_mobility_status: params.mobilityStatus,
    p_max_results: 4
  });
...
/**
 * Create an emergency booking/lead with all required details.
 * Saves all data in a single atomic transaction.
 */
export const createEmergencyBooking = async (params: {
  technicianId: string;
  ...
  const leadData = {
    ...
    service_requested: 'Emergency Roadside Assistance',
    situation: params.situation,
    vehicle_model: params.vehicleMake,
    transmission: params.transmission,
    fuel_type: params.fuelType,
    description: params.description,
    mobility_status: params.mobilityStatus,
  ...
  // Trigger notification to technician (fire and forget)
  (async () => {
    try {
      await supabase.functions.invoke('send-lead-notification', {
        body: {
          ...
          client_name: params.clientName,
          client_phone: params.clientPhone,
          service_requested: `Emergency: ${params.situation}`,
          client_location: params.location,
          vehicle_model: params.vehicleMake,
          is_emergency: true
        }
...
/**
 * Build a pre-filled WhatsApp message for emergency bookings.
 * Message assembly is done server-side to reduce client processing.
 */
export const buildEmergencyWhatsAppMessage = (params: {
  clientName: string;
  situation: string;
  location: string;
  vehicleMake: string;
  ...
  const mobilityText = 
    params.mobilityStatus === 'yes' ? 'The car can still move' :
    params.mobilityStatus === 'barely' ? 'The car can barely move (risky to drive)' :
    'The car is completely stuck';

  const message = `🚨 EMERGENCY ROADSIDE ASSISTANCE 🚨

Hi, I found you on Mekh. My name is ${params.clientName}.
...
DETAILS: ${params.description || 'No additional details provided'}

I need immediate assistance. Can you help?`;
...
/**
 * Get verification status for the current technician.
 * Returns the full verification result including badge status and progress tracker.
 */
export const getMyVerificationStatus = async () => {
  const { evaluateVerification } = await import('./verificationEngine');
  
  const user = await getCurrentUser();
  if (!user) throw new Error('Not authenticated');

  // Fetch technician profile with all required data
  ...
  const businessHours = await getTechnicianBusinessHours(tech.id);

  // Evaluate verification
  return evaluateVerification(tech, businessHours);
};
