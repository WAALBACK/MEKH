...
export interface Technician {
  ...
  tow_truck_number_plate: string | null; // Required for towing category - vehicle number plate
  trusted_brands_used: string; // Comma-separated list of trusted brands/materials used
  service_guarantee: string; // Line-separated list of service guarantees
  completed_jobs: number; // Number of jobs completed on the platform (for verified badge)
  is_verified: boolean; // Computed column indicating if technician meets all verified badge criteria
  ...
  distance_km?: number;  // Computed distance in kilometers (for emergency)
  eta_minutes?: number;  // Computed ETA in minutes (for emergency)
  business_hours?: BusinessHours[];  // Business hours schedule
  is_24_7?: boolean;  // 24/7 business hours flag
}
...
export const SERVICE_CATEGORIES: {
  ...
  },
  {
    ...
    label: 'Towing',
    icon: '',
    keywords: ['towing', 'tow', 'recovery', 'flatbed', 'jumpstart']
  }
];
