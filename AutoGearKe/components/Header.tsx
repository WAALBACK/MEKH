...
const Header: React.FC<HeaderProps> = ({
  ...
  const handleSignOut = async () => {
    try {
      await signOut();
      setMenuOpen(false);
      // Navigation will be handled by Layout.tsx after the sign-out event
    } catch (error) {
      console.error('Sign out error:', error);
      setMenuOpen(false);
      // Even if sign-out fails, navigate to clear the UI state
      navigate('/');
    }
  };
  ...
  return (
    <header style={{ paddingTop: 'env(safe-area-inset-top)' }} className="border-b border-primary-700 sticky top-0 bg-primary-600 backdrop-blur-md z-50">
          ...
          <Link to="/" className="text-xl font-black tracking-tighter text-white flex items-center group">
            <div className="relative mr-0 md:mr-4 logo-glow flex items-center justify-center">
              <img
                ...
                alt="Mekh Logo"
                className="h-10 md:h-12 max-h-12 w-auto object-contain"
                decoding="async"
                fetchPriority="high"
                width={48}
                height={48}
                ...
                <button onClick={() => setMenuOpen(!menuOpen)} className="flex items-center gap-2">
                  <div className="w-8 h-8 md:w-9 md:h-9 rounded-full bg-white flex items-center justify-center text-primary-600 font-bold text-sm overflow-hidden">
                    {technician?.profile_image ? (
                      <img src={profileThumb(technician.profile_image)} alt="Profile" className="w-full h-full object-cover" loading="lazy" decoding="async" width={36} height={36} />
                    ) : (
              ...
              {isClient && (
                <>
                  <Link to="/" className={navLink(location.pathname === '/')}>Home</Link>
                   <Link to="/roadside-emergency" className={navLink(location.pathname.startsWith('/blogs'))}>
                    SOS
                  </Link>
