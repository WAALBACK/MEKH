...
function urlEntry({ url, priority, changefreq, lastmod }) {
  const loc      = `<loc>${escapeXml(BASE_URL + url)}</loc>`;
...
export default {
  async fetch(request, env, ctx) {
    ...
    // Handle separate technician sitemap
    if (url.pathname === '/sitemap-technicians.xml') {
      // Fetch from your KV store or directly from the generated file
      const xml = await env.SITEMAP_KV.get('sitemap-technicians');
      // OR if you serve it as a static file from Pages:
      // const res = await fetch('https://mekh.app/sitemap-technicians.xml');
    ...
    const [technicians, articles, technicianServices] = await Promise.all([
      ...
      querySupabase(env, 'technician_services', 'service_name,technician_id,technicians!inner(slug,created_at,status)', {}),
    ]);
    ...
    // Service-specific technician URLs
    const serviceSpecificUrls = technicianServices
      .filter(ts => ts.technicians?.status === 'live')
      .map(ts => {
        const serviceSlug = ts.service_name.toLowerCase().replace(/\s+/g, '-');
        return {
          url:        `/technician/${serviceSlug}/${ts.technicians.slug}`,
          priority:   '0.8',
          changefreq: 'weekly',
          lastmod:    extractDate(ts.technicians.created_at),
        };
      });
